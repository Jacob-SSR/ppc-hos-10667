// app/api/accident-sheets/route.ts  — REFACTORED
...
const COLUMN_MAP: Record<string, keyof AccidentRow> = {
  ลำดับ: "no", HN: "hn", อายุ: "age", เพศ: "sex",
  วันที่มารับการรักษา: "treatDate", วันที่เกิดเหตุ: "accidentDate",
  เวลาที่เกิด: "timeSlot", เอกสารพรบ: "prb", สิทธิการรักษา: "rights",
  ประเภทพาหนะ: "vehicle", ระดับความรุนแรง: "severity", ประเภท: "transport",
  ถนนที่เกิดเหตุ: "road", ตำบลที่เกิดเหตุ: "tambon",
  "respone time กรณีมีออก EMS": "responseTime",
  วินิจฉัยทางการแพทย์: "diagnosis",
  "mechanismof accident (รายละเอียดการบาดเจ็บ)": "mechanism",
  สถานะ: "status", ดื่มสุรา: "alcohol", การป้องกัน: "protection",
  ปัจจัยเสี่ยง: "riskFactor", ตรวจแอลกอฮอล์: "alcoholTest",
  ที่อยู่: "address", status: "finalStatus", หมายเหตุ: "note",
};

export interface AccidentRow {
  no: number; hn: string; age: number; sex: string;
  treatDate: string; accidentDate: string; timeSlot: string; prb: string;
  rights: string; vehicle: string; severity: string; transport: string;
  road: string; tambon: string; responseTime: string; diagnosis: string;
  mechanism: string; status: string; alcohol: string; protection: string;
  riskFactor: string; alcoholTest: string; address: string;
...
function parseRows(rawRows: string[][]): {
  ...
  for (let i = 1; i < rawRows.length; i++) {
    ...
    rows.push({
      ...
      accidentDate: parseDate(get(row, "accidentDate")) || get(row, "accidentDate"),
...
function buildSummary(rows: AccidentRow[]) {
  ...
  const helmetWorn = rows.filter(
    (r) => r.protection.includes("สวมหมวกนิรภัย") && !r.protection.includes("ไม่"),
  ).length;
  ...
  rows.forEach((r) => {
    ...
    const key =
      t.includes("0.00-4") || t.includes("00:00") ? "00:00–04:00"
        : t.includes("4.00-") || t.includes("04:") ? "04:00–08:00"
        : (t.includes("8") || t.includes("08")) && t.includes("12") ? "08:00–12:00"
  ...
  const byAgeGroup = AGE_GROUPS.map((g) => {
    const [lo, hi] =
      g === "<15" ? [0, 14] : g === "55+" ? [55, 999] : g.split("-").map(Number);
  ...
  try {
    ...
    if (debug) {
      return NextResponse.json({
        headers,
        sampleRow,
        treatDateSamples: rows.slice(0, 10).map((r) => ({ hn: r.hn, treatDate: r.treatDate })),
