// app/api/ttm-dashboard/route.ts
// Dashboard แพทย์แผนไทย — รวมทุก section (summary, rights, icd10, queue, patients)
// รับ ?preset=today|7days|30days|thismonth  หรือ  ?start=YYYY-MM-DD&end=YYYY-MM-DD
import { NextResponse } from "next/server";
import { getTtmDashboard } from "@/lib/ttm.service";
...
export async function GET(req: Request) {
  try {
    ...
    const data = await getTtmDashboard(start, end);
    return NextResponse.json({ ...data, start, end });
  } catch (error) {
    console.error("TTM dashboard error:", error);
