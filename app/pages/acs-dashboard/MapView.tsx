...
import type { AcsMapData, AcsMapPoint } from "@/app/api/acs-map/route";

// ─── สีตามกลุ่มวินิจฉัย — ชุดเดียวกับ DX_COLOR_MAP ใน Overview ─────────────────
const DX_COLOR: Record<string, string> = {
    STEMI: "#E24B4A", // แดง — เร่งด่วนสุด
    NSTEMI: "#EF9F27", // ส้ม
    อื่นๆ: "#378ADD", // ฟ้า
    ไม่ระบุ: "#888780",
};
const dxHex = (d: string) => DX_COLOR[d] ?? "#888780";
                        ...
                        >
                            {colorMode && (
                                <span
                                    ...
                                    style={{
                                        background: active ? "#fff" : dxHex(opt),
                                    }}
                                />
                            )}
                            {opt || "ไม่ระบุ"}
...
// ─── Page ──────────────────────────────────────────────────────────────────────
export default function AcsMapView() {
    const { data, loading, error } = useFetchOnce<AcsMapData>("/api/acs-map");

    // ── ตัวกรอง ──
    const [q, setQ] = useState("");
    const [fDx, setFDx] = useState<Set<string>>(new Set());
    ...
    const [fSk, setFSk] = useState<Set<string>>(new Set());
    ...
    // ── จุดที่ผ่านตัวกรอง ──
    const filtered = useMemo<AcsMapPoint[]>(() => {
        if (!data) return [];
        const term = q.trim().toLowerCase();
        return data.points.filter((p) => {
            if (fDx.size && !fDx.has(p.dxGroup)) return false;
            if (fYear.size && !fYear.has(p.yearBE)) return false;
            if (fArea.size && !fArea.has(p.area)) return false;
            if (fEms.size && !fEms.has(p.ems1669)) return false;
            if (fSk.size && !fSk.has(p.skGiven)) return false;
            if (fStatus.size && !fStatus.has(p.status)) return false;
            if (fMonth.size && !fMonth.has(p.serviceMonth)) return false;
            if (term) {
                const hay =
                    `${p.fullName} ${p.hn} ${p.area} ${p.diagnosis} ${p.definiteDiagnosis} ${p.underlying}`.toLowerCase();
                if (!hay.includes(term)) return false;
            }
            return true;
        });
    }, [data, q, fDx, fYear, fArea, fEms, fSk, fStatus, fMonth]);
    ...
    useEffect(() => {
        ...
        for (const p of filtered) {
            const hex = dxHex(p.dxGroup);
            // เคสเสียชีวิต: หมุดขอบดำหนา ให้สะดุดตากว่าเคสอื่น
            const icon = L.divIcon({
                className: "acs-pin",
                html: `<span style="display:block;width:16px;height:16px;border-radius:50%;background:${hex};border:2.5px solid ${p.isDead ? "#1f1f1f" : "#fff"};box-shadow:0 1px 4px rgba(0,0,0,.4)"></span>`,
                iconSize: [16, 16],
                iconAnchor: [8, 8],
            });
        ...
        <div style="min-width:220px;font-family:'Prompt','Noto Sans Thai',sans-serif">
          ...
          <div style="display:inline-block;font-size:11px;color:#fff;background:${hex};padding:1px 8px;border-radius:999px;margin-bottom:6px">${esc(p.dxGroup === "อื่นๆ" && p.diagnosis ? p.diagnosis : p.dxGroup)}</div>
          <div style="font-size:12px;color:#444;line-height:1.5">
            <div>🏥 ปี ${esc(p.yearBE || "-")}${p.age != null ? ` · อายุ ${p.age} ปี` : ""} · ${esc(p.area || "-")}</div>
            ${p.underlying ? `<div>💊 โรคประจำตัว: ${esc(p.underlying)}</div>` : ""}
            <div>${p.ekgMin != null ? `📈 EKG ${p.ekgMin} นาที · ` : ""}SK: <b>${esc(p.skGiven)}</b> · EMS 1669: ${esc(p.ems1669)}</div>
            ${p.definiteDiagnosis ? `<div>🩻 Definite Dx: ${esc(p.definiteDiagnosis)}</div>` : ""}
            <div>🩺 สถานะ: <b>${esc(p.status)}</b>${p.outcome && p.outcome !== "ไม่ระบุ" ? ` · ผลลัพธ์: <b style="color:${p.isDead ? "#A32D2D" : "#3B6D11"}">${esc(p.outcome)}</b>` : ""}</div>
    ...
    const focusPoint = (p: AcsMapPoint) => {
        setSelectedId(p.id);
        const map = mapRef.current;
                ...
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        <h1 className="text-base font-bold text-gray-800 leading-tight">
                            แผนที่บ้านผู้ป่วย ACS
                        </h1>
                        <p className="text-[11px] text-gray-500">
                            โรคหลอดเลือดหัวใจ · พิกัดหลังคาเรือน อ.พลับพลาชัย จ.บุรีรัมย์
                        </p>
                    </div>
                </div>
                    ...
                    <div className="p-3 border-b">
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder="ค้นหาชื่อ / HN / วินิจฉัย / พื้นที่"
                    ...
                    <div className="p-3 border-b overflow-y-auto max-h-[45%]">
                        {s && (
                            <>
                                <FilterChips
                                    label="กลุ่มวินิจฉัย"
                                    options={s.filters.dxGroup}
                                    selected={fDx}
                                    onToggle={toggle(setFDx)}
                                    colorMode
                                />
                                <FilterChips
                                    label="ปี พ.ศ."
                                ...
                                <FilterChips
                                    label="พื้นที่"
                                    options={s.filters.area}
                                    selected={fArea}
                                    onToggle={toggle(setFArea)}
                                />
                                <FilterChips
                                    label="ได้ยา SK"
                                    options={s.filters.sk}
                                    selected={fSk}
                                    onToggle={toggle(setFSk)}
                                />
                                <FilterChips
                            ...
                            >
                                ...
                                <div className="min-w-0">
                                    ...
                                    <div className="text-[10px] text-gray-400 mt-0.5 flex flex-wrap gap-x-2">
                                        <span
                                            style={{
                                                color: p.isDead ? "#A32D2D" : undefined,
                                                fontWeight: p.isDead ? 600 : undefined,
                                            }}
                                        >
                                            {p.isDead ? "เสียชีวิต" : p.status}
                                        </span>
                                        {p.skGiven === "Yes" && (
                                            <span className="text-teal-600">· SK</span>
                                        )}
                                        {p.yearBE && <span>· ปี {p.yearBE}</span>}
                                    </div>
                                </div>
                            </button>
                        ))}
                ...
                <div className="flex-1 relative z-0 isolate min-w-0">
                    ...
                    <div className="absolute bottom-4 right-4 z-[500] bg-white/95 backdrop-blur shadow-md rounded-lg px-3 py-2.5">
                        ...
                        <div className="space-y-1">
                            {Object.entries(DX_COLOR)
                                .filter(([k]) => k !== "ไม่ระบุ")
                                .map(([dx, hex]) => (
                                    <div key={dx} className="flex items-center gap-1.5">
                                        <span
                                            className="w-3 h-3 rounded-full border border-white shadow"
                                            style={{ background: hex }}
                                        />
                                        <span className="text-[11px] text-gray-600">{dx}</span>
                                    </div>
                                ))}
