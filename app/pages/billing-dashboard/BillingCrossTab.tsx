...
function buildRows(data: BillingDashboardData): RowData[] {
  return data.units.map((unit) => {
    ...
    for (const svc of SERVICE_COLS) {
      const matching = unit.items.filter((i: BillingItemSummary) => i.รายการขอเบิก === svc.key);
      services[svc.key] = {
        ...
        compCount: matching.filter((i: BillingItemSummary) => i.สถานะ === "ชดเชย").reduce((s: number, i: BillingItemSummary) => s + i.จำนวน, 0),
...
export default function BillingCrossTab({ data }: Props) {
  ...
  const totals = useMemo(() => {
    const t = { totalBaht: 0, ...Object.fromEntries(SERVICE_COLS.map(s => [s.key, { claimCount:0, claimBaht:0, compCount:0, compBaht:0 }])) } as Record<string, SvcStat> & { totalBaht: number };
    for (const svc of SERVICE_COLS) {
      t[svc.key] = { claimCount: 0, claimBaht: 0, compCount: 0, compBaht: 0 };
    }
    for (const row of rows) {
      t.totalBaht += row.totalBaht;
      for (const svc of SERVICE_COLS) {
        const s = row.services[svc.key];
        t[svc.key].claimCount += s.claimCount;
        t[svc.key].claimBaht += s.claimBaht;
        t[svc.key].compCount += s.compCount;
        t[svc.key].compBaht += s.compBaht;
      }
    }
    return t;
  }, [rows]);
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-4">
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-sm font-bold text-gray-600">ตารางสรุปการเรียกเก็บและชดเชย — แยกตามหน่วยบริการและประเภทบริการ</h4>
      </div>
      <div className="overflow-x-auto">
        <table className="border-collapse text-xs" style={{ minWidth: "900px" }}>
          <thead>
            <tr>
              <th className={thTop} rowSpan={3} style={{ minWidth: 56 }}>รหัส<br />หน่วยบริการ</th>
              <th className={thTop} rowSpan={3} style={{ minWidth: 130, textAlign: "left" }}>ชื่อหน่วยบริการ</th>
              <th className={thTop} rowSpan={3} style={{ minWidth: 76 }}>รวม (บาท)</th>
              {SERVICE_COLS.map((s) => (
            ...
            <tr>
              {SERVICE_COLS.map((s) => (
                <>
                  <th key={`${s.key}-c`} className={thMid} colSpan={2}>เรียกเก็บ</th>
                  <th key={`${s.key}-o`} className={thMid} colSpan={2}>ชดเชย</th>
                </>
              ))}
            </tr>
            <tr>
              {SERVICE_COLS.map((s) => (
                <>
                  <th key={`${s.key}-cn`} className={thSub}>รายการ</th>
                  <th key={`${s.key}-cb`} className={thSub}>บาท</th>
                  <th key={`${s.key}-on`} className={thSub}>รายการ</th>
                  <th key={`${s.key}-ob`} className={thSub}>บาท</th>
                </>
              ))}
            </tr>
          </thead>
          ...
          <tfoot>
            <tr className="bg-[#d6f0e0] border-t-2 border-[#55b882]">
              ...
              {SERVICE_COLS.map((s) => {
                const t = (totals as Record<string, SvcStat>)[s.key];
                return (
                  <>
                    <td key={`${s.key}-tc`} className="px-2 py-2 text-right tabular-nums text-xs font-medium text-[#1a5233]">{fmtN(t.claimCount)}</td>
                    <td key={`${s.key}-tb`} className="px-2 py-2 text-right tabular-nums text-xs font-medium text-[#1a5233] border-r-2 border-gray-300">{fmtB(t.claimBaht)}</td>
                    <td key={`${s.key}-tc2`} className="px-2 py-2 text-right tabular-nums text-xs font-medium text-[#1a5233]">{fmtN(t.compCount)}</td>
                    <td key={`${s.key}-tb2`} className="px-2 py-2 text-right tabular-nums text-xs font-medium text-[#1a5233] border-r-2 border-gray-300">{fmtB(t.compBaht)}</td>
                  </>
                );
              })}
            </tr>
