...
import type { SepsisMapData, SepsisMapPoint } from "@/app/api/sepsis-map/route";

// ─── สีตามตำแหน่งติดเชื้อ (Site of infection) — ชุดเดียวกับกราฟใน Overview ────
...
const siteHex = (s: string) => SITE_COLORS[s] ?? "#888780";
                        ...
                        >
                            {colorMode && (
                                <span
                                    ...
                                    style={{
                                        background: active ? "#fff" : siteHex(opt),
                                    }}
                                />
                            )}
                            {opt || "ไม่ระบุ"}
...
// ─── Page ──────────────────────────────────────────────────────────────────────
export default function SepsisMapView() {
    const { data, loading, error } =
        useFetchOnce<SepsisMapData>("/api/sepsis-map");

    // ── ตัวกรอง ──
    const [q, setQ] = useState("");
    const [fYear, setFYear] = useState<Set<string>>(new Set());
    const [fSite, setFSite] = useState<Set<string>>(new Set());
    ...
    // ── จุดที่ผ่านตัวกรอง ──
    const filtered = useMemo<SepsisMapPoint[]>(() => {
        if (!data) return [];
        const term = q.trim().toLowerCase();
        return data.points.filter((p) => {
            if (fYear.size && !fYear.has(p.year)) return false;
            if (fSite.size && !fSite.has(p.siteOfInfection)) return false;
            if (fStatus.size && !fStatus.has(p.definiteStatus)) return false;
            if (fType.size && !fType.has(p.typeOfInfection)) return false;
            if (fTambon.size && !fTambon.has(p.tambon)) return false;
            if (fMonth.size && !fMonth.has(p.serviceMonth)) return false;
            if (term) {
                const hay =
                    `${p.fullName} ${p.hn} ${p.address} ${p.diagnosis} ${p.pathogen}`.toLowerCase();
                if (!hay.includes(term)) return false;
            }
            return true;
        });
    }, [data, q, fYear, fSite, fStatus, fType, fTambon, fMonth]);
    ...
    useEffect(() => {
        ...
        for (const p of filtered) {
            const hex = siteHex(p.siteOfInfection);
            const dead = p.definiteStatus === "Dead";
            // เคสเสียชีวิต: หมุดขอบดำหนา ให้สะดุดตากว่าเคส Improve
            const icon = L.divIcon({
                className: "sepsis-pin",
            ...
            const statusColor = dead ? "#A32D2D" : "#3B6D11";
            const popup = `
        <div style="min-width:220px;font-family:'Prompt','Noto Sans Thai',sans-serif">
          ...
          <div style="display:inline-block;font-size:11px;color:#fff;background:${hex};padding:1px 8px;border-radius:999px;margin-bottom:6px">${esc(p.siteOfInfection)}</div>
          <div style="font-size:12px;color:#444;line-height:1.5">
            <div>🏥 ปีงบ ${esc(p.year || "-")} · ${esc(p.department || "-")}${p.age != null ? ` · อายุ ${p.age} ปี` : ""}</div>
            <div>🏠 ${esc(p.address || `ต.${p.tambon}` || "-")}</div>
            <div>🦠 ${esc(p.typeOfInfection || "-")}${p.pathogen && p.pathogen !== "ไม่ระบุ" ? ` · ${esc(p.pathogen)}` : ""}</div>
            ${p.septicShock === true ? `<div>⚠️ <b style="color:#A32D2D">Septic shock</b></div>` : ""}
            <div>🩺 ผลลัพธ์: <b style="color:${statusColor}">${esc(p.definiteStatus || "-")}</b></div>
    ...
    const focusPoint = (p: SepsisMapPoint) => {
        setSelectedId(p.id);
        const map = mapRef.current;
                ...
                >
                    ...
                    <div className="p-3 border-b">
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder="ค้นหาชื่อ / HN / เชื้อ / ที่อยู่"
                    ...
                    <div className="p-3 border-b overflow-y-auto max-h-[45%]">
                        {s && (
                            <>
                                <FilterChips
                                    label="ตำแหน่งติดเชื้อ (Site)"
                                    options={s.filters.site}
                                    selected={fSite}
                                    onToggle={toggle(setFSite)}
                                    colorMode
                                />
                                <FilterChips
                                    label="ปีงบประมาณ"
                                ...
                                />
                                <FilterChips
                                    label="ผลลัพธ์ (Improve/Dead)"
                                    options={s.filters.status}
                                    selected={fStatus}
                                    onToggle={toggle(setFStatus)}
                                />
                                <FilterChips
                                    label="ชนิดการติดเชื้อ"
                                    options={s.filters.typeInf}
                                    selected={fType}
                        ...
                        {filtered.map((p) => (
                            ...
                            >
                                <span
                                    ...
                                    style={{
                                        background: siteHex(p.siteOfInfection),
                                        outline:
                                            p.definiteStatus === "Dead"
                                                ? "2px solid #1f1f1f"
                                                : undefined,
                                    }}
                                />
                                            ...
                                            style={{
                                                color:
                                                    p.definiteStatus === "Dead"
                                                        ...
                                                        : undefined,
                                                fontWeight:
                                                    p.definiteStatus === "Dead" ? 600 : undefined,
                                            }}
                                        >
                                            {p.definiteStatus}
                                        </span>
                                        {p.septicShock === true && (
                                            <span className="text-red-500">· Shock</span>
                                        )}
                                        {p.year && <span>· ปีงบ {p.year}</span>}
                                    </div>
                                </div>
                            </button>
                        ))}
                ...
                <div className="flex-1 relative z-0 isolate min-w-0">
                    ...
                    <div className="absolute bottom-4 right-4 z-[500] bg-white/95 backdrop-blur shadow-md rounded-lg px-3 py-2.5">
                        <div className="text-[10px] font-semibold text-gray-500 mb-1.5">
                            ตำแหน่งติดเชื้อ (Site)
                        </div>
                        <div className="space-y-1">
                            {Object.entries(SITE_COLORS)
                                .filter(([k]) => k !== "ไม่ระบุ")
                                .map(([site, hex]) => (
