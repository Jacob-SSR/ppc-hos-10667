...
import type { ImcMapData, ImcMapPoint } from "@/app/api/imc-map/route";

// ─── สีตามกลุ่มโรค (Dx.) — ชุดเดียวกับกราฟใน Overview ─────────────────────────
...
const dxHex = (d: string) => DX_COLOR[d] ?? "#8b8b8b";
...
// ─── Page ──────────────────────────────────────────────────────────────────────
export default function ImcMapView() {
    const { data, loading, error } = useFetchOnce<ImcMapData>("/api/imc-map");
    ...
    // ── จุดที่ผ่านตัวกรอง ──
    const filtered = useMemo<ImcMapPoint[]>(() => {
        if (!data) return [];
        const term = q.trim().toLowerCase();
        return data.points.filter((p) => {
            if (fYear.size && !fYear.has(p.fiscalYear)) return false;
            if (fDx.size && !fDx.has(p.diagnosis)) return false;
            if (fTambon.size && !fTambon.has(p.tambon)) return false;
            if (fChannel.size && !fChannel.has(p.channel)) return false;
            if (fStatus.size && !fStatus.has(p.finalStatus)) return false;
            if (fMonth.size && !fMonth.has(p.serviceMonth)) return false;
            if (term) {
                const hay =
                    `${p.fullName} ${p.hn} ${p.an} ${p.tambon} ${p.diagnosis}`.toLowerCase();
                if (!hay.includes(term)) return false;
            }
            return true;
        });
    }, [data, q, fYear, fDx, fTambon, fChannel, fStatus, fMonth]);
    ...
    useEffect(() => {
        ...
        for (const p of filtered) {
            ...
            const biLine =
                p.biAdmit != null || p.biBeforeDc != null
                    ? `<div>📊 BI แรกรับ <b>${p.biAdmit ?? "-"}</b> → ก่อนกลับบ้าน <b>${p.biBeforeDc ?? "-"}</b>${p.biAfterDc != null ? ` → หลัง D/C <b>${p.biAfterDc}</b>` : ""}</div>`
                    : "";
        ...
        <div style="min-width:220px;font-family:'Prompt','Noto Sans Thai',sans-serif">
          ...
          <div style="display:inline-block;font-size:11px;color:#fff;background:${hex};padding:1px 8px;border-radius:999px;margin-bottom:6px">${esc(p.diagnosis)}</div>
          <div style="font-size:12px;color:#444;line-height:1.5">
            <div>🏥 ปีงบ ${esc(p.fiscalYear || "-")} · ${esc(p.channel || "-")}${p.age != null ? ` · อายุ ${p.age} ปี` : ""}</div>
            <div>📍 ต.${esc(p.tambon || "-")}</div>
            ${biLine}
            <div>🩺 สถานะหลัง D/C: <b>${esc(p.finalStatus || "-")}</b>${p.complication && p.complication !== "No" ? ` · ⚠️ ${esc(p.complication)}` : ""}</div>
            ${p.admitDate ? `<div>🗓️ Admit ${esc(p.admitDate)}${p.dcDate ? ` → D/C ${esc(p.dcDate)}` : ""}</div>` : ""}
            ${p.hn ? `<div style="color:#888">HN ${esc(p.hn)}${p.an ? ` · AN ${esc(p.an)}` : ""}</div>` : ""}
          </div>
          ${p.mapLink
    ...
    const focusPoint = (p: ImcMapPoint) => {
        setSelectedId(p.id);
        const map = mapRef.current;
                ...
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        <h1 className="text-base font-bold text-gray-800 leading-tight">
                            แผนที่บ้านผู้ป่วย IMC
                        </h1>
                        <p className="text-[11px] text-gray-500">
                            การดูแลระยะกลาง · พิกัดหลังคาเรือน อ.พลับพลาชัย จ.บุรีรัมย์
                        </p>
                    </div>
                </div>
                    ...
                    <div className="p-3 border-b">
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder="ค้นหาชื่อ / HN / AN / โรค"
                    ...
                    <div className="p-3 border-b overflow-y-auto max-h-[45%]">
                        {s && (
                            <>
                                ...
                                />
                                <FilterChips
                                    label="ช่องทางรับ (Referback/walkin)"
                                    options={s.filters.channel}
                                    selected={fChannel}
                                    onToggle={toggle(setFChannel)}
                                />
                                <FilterChips
                                    label="สถานะหลัง D/C"
                        ...
                        {filtered.map((p) => (
                            ...
                            >
                                <span
                                    ...
                                    style={{ background: dxHex(p.diagnosis) }}
                                />
                                <div className="min-w-0">
                                    ...
                                    <div className="text-[11px] text-gray-500 truncate">
                                        {p.diagnosis} · ต.{p.tambon || "-"}
                                    </div>
                                    <div className="text-[10px] text-gray-400 mt-0.5 flex flex-wrap gap-x-2">
                                        {p.finalStatus && <span>{p.finalStatus}</span>}
                                        {(p.biAdmit != null || p.biBeforeDc != null) && (
                                            <span>
                                                · BI {p.biAdmit ?? "-"}→{p.biBeforeDc ?? "-"}
                                            </span>
                                        )}
                                    </div>
                                </div>
                ...
                <div className="flex-1 relative z-0 isolate min-w-0">
                    ...
                    <div className="absolute bottom-4 right-4 z-[500] bg-white/95 backdrop-blur shadow-md rounded-lg px-3 py-2.5">
                        ...
                        <div className="space-y-1">
                            {Object.entries(DX_COLOR).map(([dx, hex]) => (
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                />
                                <span className="text-[11px] text-gray-600">อื่นๆ</span>
                            </div>
                        </div>
                    </div>
