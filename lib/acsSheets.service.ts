// lib/acsSheets.service.ts
// Parser ทะเบียนโรคหลอดเลือดหัวใจ (ACS) — ย้ายมาจาก app/api/acs-sheets/route.ts
// เพื่อให้ /api/acs-sheets และ /api/acs-map ใช้ข้อมูล (และ cache) ชุดเดียวกัน
...
// ─── Public API ───────────────────────────────────────────────────────────────
/** payload รายปี (KPI + rows + summary) ผ่าน cache กลาง — key เดิม ["acs-sheets", year]
 *  ทั้ง /api/acs-sheets และผู้เรียกอื่นใช้ตัวนี้ → แชร์ cache ก้อนเดียว */
export async function getAcsSheetsCached(year: string): Promise<AcsSheetsData> {
  return cachedQuery(
...
// ─── ผู้ป่วยทุกปี (สำหรับแผนที่) ───────────────────────────────────────────────
export interface AcsPatientWithYear extends AcsPatient {
  yearBE: string; // ปี พ.ศ. ตามชื่อชีต เช่น "2569"
}

async function buildAllRows(): Promise<{
  updatedAt: string;
  years: string[];
  rows: AcsPatientWithYear[];
}> {
  const sheets = await getSheetClient();
  const titles = await getAllSheetTitles(sheets, SPREADSHEET_ID);
  const years = titles
  ...
  const rows: AcsPatientWithYear[] = [];
  await Promise.all(
    years.map(async (y) => {
      const raw = await getValues(sheets, SPREADSHEET_ID, `'${y}'!A4:AB200`);
      raw
        .filter((r) => toStr(r[1]) !== "")
        .forEach((r) => rows.push({ ...parsePatient(r), yearBE: y }));
    }),
  );

  return { updatedAt: new Date().toISOString(), years, rows };
}

/** ผู้ป่วย ACS ทุกปีรวมกัน (ใช้ทำแผนที่) — cache แยกจาก payload รายปี */
export async function getAcsAllRowsCached() {
  return cachedQuery(["acs-all-rows"], buildAllRows, TTL_RESULT * 2);
}
