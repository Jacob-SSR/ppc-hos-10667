// lib/sepsisSheets.service.ts
// Parser + summary ของทะเบียน Sepsis — ย้ายมาจาก app/api/sepsis-sheets/route.ts
// เพื่อให้ /api/sepsis-sheets และ /api/sepsis-map ใช้ข้อมูล (และ cache) ชุดเดียวกัน
import {
  getSheetClient,
  getAllSheetTitles,
  getValues,
...
import { getAddressByHn, type AddressParts } from "./sepsis.service";
...
// ─── Public API ───────────────────────────────────────────────────────────────
export type SepsisPayload = Awaited<ReturnType<typeof buildPayload>>;

/** payload เต็ม (rows + summary) ผ่าน cache กลาง — key เดิม "sepsis-sheets"
 *  ทั้ง /api/sepsis-sheets และ /api/sepsis-map เรียกตัวนี้ → แชร์ cache ก้อนเดียว */
export async function getSepsisSheetsCached(): Promise<SepsisPayload> {
  return cachedQuery(["sepsis-sheets"], buildPayload, TTL_RESULT);
}
