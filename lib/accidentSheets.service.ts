// lib/accidentSheets.service.ts
// Parser + summary ของทะเบียนอุบัติเหตุ — ย้ายมาจาก app/api/accident-sheets/route.ts
// เพื่อให้ /api/accident-sheets และ /api/accident-map ใช้ข้อมูล (และ cache) ชุดเดียวกัน
...
// ─── Public API ───────────────────────────────────────────────────────────────
export type AccidentPayload = Awaited<ReturnType<typeof buildAccidentData>>;

/** payload เต็ม (rows + summary + debug fields) ผ่าน cache กลาง — key เดิม "accident-sheets"
 *  ทั้ง /api/accident-sheets และ /api/accident-map เรียกตัวนี้ → แชร์ cache ก้อนเดียว */
export async function getAccidentSheetsCached(): Promise<AccidentPayload> {
  return cachedQuery([CACHE_KEY], buildAccidentData, TTL_SECONDS);
}
